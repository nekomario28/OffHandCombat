...
LOG_FILE="client-interaction-e2e.log"
SUCCESS_MARKER="Off Hand Combat interaction priority E2E passed: button, door and chest"
FAILURE_MARKER="Off Hand Combat interaction priority E2E failed"
WORLD_SOURCE="run/world"
WORLD_DESTINATION="run/saves/InteractionWorld"
RUN_PID=""
RESULT="failed"
...
setsid stdbuf -oL -eL xvfb-run -a -s "-screen 0 1280x720x24" \
  gradle --no-daemon runClientInteractionE2E --stacktrace \
...
if [[ "$RESULT" != "pass" ]]; then
  cat "$LOG_FILE"
  echo "Interaction priority E2E did not pass: $RESULT" >&2
  exit 1
fi

if grep -Eiq 'mixin apply failed|exception in thread|fatal error|failed to load mod|crash report' "$LOG_FILE"; then
  cat "$LOG_FILE"
  echo "Interaction priority E2E found a fatal signature" >&2
  exit 1
fi

grep -F "Off Hand Combat interaction priority E2E passed for button" "$LOG_FILE"
grep -F "Off Hand Combat interaction priority E2E passed for door" "$LOG_FILE"
grep -F "Off Hand Combat interaction priority E2E passed for chest" "$LOG_FILE"
